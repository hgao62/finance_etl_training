...
def get_top_bottom_days(stock_history: pd.DataFrame, ticker:str, top_n: int = 5, ) -> pd.DataFrame:
    ticker_df = stock_history[stock_history["ticker"] == ticker]
    top_n = ticker_df.nlargest(top_n, "close")
    bottom_n = ticker_df.nsmallest(top_n, "close")
...
def filter_significant_volume(
    stock_history: pd.DataFrame, threshold: int = 1_000_000
) -> pd.DataFrame:
    """
...
def group_by_sector(stock_history: pd.DataFrame) -> pd.DataFrame:
    ...
    stock_history_grouped = (
        stock_history.groupby("sector")
        .agg({"close": "mean", "volume": "mean"})
        .reset_index()
    )

    stock_history_grouped = stock_history_grouped.rename(
        columns={"close": "avg_close", "volume": "avg_volume"}
    )
